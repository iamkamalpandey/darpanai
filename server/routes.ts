...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Authentication routes for login/logout with proper routing
  app.post("/api/auth/login", (req: Request, res: Response, next: NextFunction) => {
    ...
    passport.authenticate('local', (err: Error | null, user: Express.User | false, info: { message: string }) => {
      ...
      req.login(user, (loginErr) => {
        ...
        // Return user data with role-based routing information
        res.json({
          user,
          redirectTo: user.role === 'admin' ? '/admin' : user.role === 'expert' ? '/expert' : '/',
          success: true
        });
      });
    })(req, res, next);
  ...
  // Expert routes for assigned students management
  app.get("/api/expert/stats", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Get expert's assigned students from database
      const assignedStudents = await storage.getExpertAssignedStudents(expertId);
      const activeStudents = assignedStudents.filter(s => s.status === 'active');
      const pendingConsultations = await storage.getExpertPendingConsultations(expertId);
      
      const stats = {
        totalStudents: assignedStudents.length,
        activeStudents: activeStudents.length,
        completedAssignments: assignedStudents.filter(s => s.status === 'completed').length,
        pendingConsultations: pendingConsultations.length
      };
      
      console.log(`Expert ${expertId} stats:`, stats);
      res.json(stats);
    } catch (error) {
  ...
  app.get("/api/expert/students/recent", requireAuth, async (req: Request, res: Response) => {
    try {
      if (req.user?.role !== 'expert') {
        return res.status(403).json({ error: 'Expert access required' });
      }
      ...
      // Get recent assigned students from database
      const recentStudents = await storage.getExpertRecentStudents(expertId);
      
      console.log(`Expert ${expertId} recent students:`, recentStudents.length, 'students');
      res.json(recentStudents);
    } catch (error) {
      console.error('Expert recent students error:', error);
      res.status(500).json({ error: 'Failed to fetch recent students' });
    }
  });

  app.get("/api/expert/consultations/pending", requireAuth, async (req: Request, res: Response) => {
    try {
      if (req.user?.role !== 'expert') {
        return res.status(403).json({ error: 'Expert access required' });
      }
      ...
      // Get pending consultations from database
      const pendingConsultations = await storage.getExpertPendingConsultations(expertId);
      
      console.log(`Expert ${expertId} pending consultations:`, pendingConsultations.length, 'consultations');
      res.json(pendingConsultations);
    } catch (error) {
      console.error('Expert pending consultations error:', error);
      res.status(500).json({ error: 'Failed to fetch pending consultations' });
    }
  });

  app.get("/api/expert/students", requireAuth, async (req: Request, res: Response) => {
    try {
      if (req.user?.role !== 'expert') {
        return res.status(403).json({ error: 'Expert access required' });
      }
      ...
      // Get all assigned students with filtering and pagination
      const result = await storage.getExpertStudentsWithFiltering(expertId, {
      ...
      console.log(`Expert ${expertId} students (filtered):`, result.students.length, 'students, total:', result.total);
      res.json(result);
    } catch (error) {
      console.error('Expert students error:', error);
      res.status(500).json({ error: 'Failed to fetch assigned students' });
    }
  });

  // Admin routes for assigning students to experts
  app.post("/api/admin/assign-student", requireAuth, async (req: Request, res: Response) => {
    try {
      if (req.user?.role !== 'admin') {
        return res.status(403).json({ error: 'Admin access required' });
      }

      const { studentId, expertId, priority = 'normal', assignmentReason } = req.body;
      const assignedBy = req.user.id;
      
      // Create student-expert assignment
      const assignment = await storage.createStudentExpertAssignment({
        studentId,
        expertId,
        assignedBy,
        priority,
      ...
      console.log(`Admin ${assignedBy} assigned student ${studentId} to expert ${expertId}`);
      res.json(assignment);
    } catch (error) {
      console.error('Student assignment error:', error);
      res.status(500).json({ error: 'Failed to assign student to expert' });
    }
  });

  app.get("/api/admin/unassigned-students", requireAuth, async (req: Request, res: Response) => {
    try {
      if (req.user?.role !== 'admin') {
        return res.status(403).json({ error: 'Admin access required' });
      }

      // Get students without expert assignments
  ...
  app.get("/api/admin/experts", requireAuth, async (req: Request, res: Response) => {
    try {
      if (req.user?.role !== 'admin') {
        return res.status(403).json({ error: 'Admin access required' });
      }
      ...
      const experts = await storage.getAvailableExperts();
      
      console.log('Available experts:', experts.length);
      res.json(experts);
    } catch (error) {
      console.error('Available experts error:', error);
