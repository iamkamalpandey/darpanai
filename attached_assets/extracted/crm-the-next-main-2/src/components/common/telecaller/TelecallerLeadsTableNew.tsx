...
type Action =
  ...
  | { type: "SET_FOLLOWUP_DATE"; followupDate: Date | null };
...
function leadsReducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "SET_FOLLOWUP_DATE":
      return { ...state, followupDate: action.followupDate, currentPage: 1 };
    default:
      return state;
  }
}
...
export default function TelecallerLeadsTableNew({
  ...
  const renderLeadRow = (lead: Lead) => {
    ...
    const leadFollowupDate =
      isFollowup && lead.followUpDates[0]
        ...
        : null;
    const isMissed =
      isFollowup && leadFollowupDate && leadFollowupDate < new Date();
    ...
    return (
      <tr key={lead.id}>
        ...
        {isFollowup && (
          <td className="whitespace-nowrap px-3 py-4 text-sm text-center text-gray-500">
            {leadFollowupDate
              ? formatDate(leadFollowupDate.toISOString())
              : "N/A"}
          </td>
        )}
        {isFollowup && (
        ...
        <div className="mb-4 flex gap-2 items-center">
          ...
          <div className="ml-auto flex items-center gap-2">
            <Popover>
              ...
              <PopoverContent className=" p-0 mr-2.5" align="start">
                <Calendar
                  mode="single"
            ...
            {followupDate && (
              <Button
                variant="outline"
                onClick={() =>
                  dispatch({ type: "SET_FOLLOWUP_DATE", followupDate: null })
                }
              >
                Clear Date
              </Button>
