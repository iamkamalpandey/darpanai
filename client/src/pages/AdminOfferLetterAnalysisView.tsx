...
interface AdminOfferLetterAnalysisData {
  id: number;
  userId: number;
  fileName: string;
  fileSize: number;
...
export default function AdminOfferLetterAnalysisView() {
  const { id } = useParams<{ id: string }>();
  const [, setLocation] = useLocation();

  const { data: analysis, isLoading, error } = useQuery<AdminOfferLetterAnalysisData>({
    queryKey: ['/api/admin/offer-letter-analyses', id],
    enabled: !!id,
  });
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-lg border border-green-200">
          <div className="flex items-center justify-between mb-4">
            <Button
              variant="ghost"
              onClick={() => setLocation('/admin/analyses')}
              className="text-green-700 hover:text-green-800"
            >
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back to All Analyses
            </Button>
            <Badge className="bg-green-100 text-green-800">Admin View</Badge>
          </div>
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white p-4 rounded-lg border">
                  ...
                  <p className="text-sm text-gray-600">{analysis.user.firstName} {analysis.user.lastName}</p>
                </div>
                <div className="bg-white p-4 rounded-lg border">
                  <div className="flex items-center gap-2 mb-2">
